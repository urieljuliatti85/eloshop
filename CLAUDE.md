...
- **Fase 22** — vínculo OAuth do vendedor com o Mercado Pago implementado (o provedor faz o KYC, a EloShop não coleta documentos, tokens ficam cifrados) **e já validado ponta a ponta**: a aplicação Marketplace está configurada (`MERCADO_PAGO_MARKETPLACE_*` e `PAYMENT_GATEWAY=mercado_pago` definidos na Railway) e há dois vendedores conectados em produção — um `live_mode: true` e um de sandbox (`Ateliê do Mercado Pago`, login `uriel@artesao.com.br`), conectado em 2026-09-06. O que falta é só a **Public Key**: ambos conectaram antes de a Fase 24 introduzir `Seller#mercado_pago_public_key`, então precisam reconectar para o cartão funcionar; o PIX não depende disso. `/admin/contas-de-teste-mercado-pago` é um cofre **opcional** de credenciais TESTUSER e está vazio — isso **não** indica ausência de conta de teste, e já induziu a erro.
