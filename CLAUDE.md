...
- **Fase 20, Etapa B** — `Gateways::MercadoPago` existe e só é selecionado com `PAYMENT_GATEWAY=mercado_pago`; em produção, configuração ausente ou gateway fake falha explicitamente. PIX vencido, idempotência por tentativa e confirmação via webhook estão implementados. Os testes automatizados (Minitest/RSpec) stubam HTTP e cobrem o contrato do adapter, nunca a API real — mas **a integração já rodou contra o sandbox de verdade**: passou por um 500 opaco e um 403 investigados a fundo com o suporte do Mercado Pago (ver `docs/payments.md`, "Cartão de crédito (Fase 24)"), e desde 2026-09-22 PIX e cartão completam o checkout sem erro, local e em produção. Falta observar uma aprovação final de pagamento ponta a ponta (pedido chegando a `paid`/`authorized` com split e comissão conferidos) — não é mais bloqueio de credencial.
