...
- **Fase 20, Etapa B** — `Gateways::MercadoPago` existe e só é selecionado com `PAYMENT_GATEWAY=mercado_pago`; em produção, configuração ausente ou gateway fake falha explicitamente. PIX vencido, idempotência por tentativa e confirmação via webhook estão implementados. Os testes stubam HTTP e cobrem o contrato do adapter, **nunca a API real** — a integração jamais rodou contra o sandbox. Bloqueada por dependência externa (credenciais do Mercado Pago), não por código.
...
- **Frete real** — `Shipping::Calculator` hoje é uma tabela interna (R$ 15 base + R$ 5/kg, prazo por faixa de CEP), sem relação com custo real de envio. ADR 005 (`docs/decisions/005-shipping-provider.md`) decidiu o provedor — **Melhor Envio**, com carteira própria por vendedor — em 2026-09-06, mas nenhuma integração foi escrita ainda; etiqueta e frete grátis seguem como decisões de negócio pendentes. `Seller` já tem endereço de origem e `Product` exige peso/dimensões para publicar, então falta só o provedor, não a fundação.
