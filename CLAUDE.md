...
Aplicação Rails 8.1 / Ruby 3.3.11 / PostgreSQL em desenvolvimento ativo. Fases 0–19 estão implementadas (catálogo, carrinho, checkout, pedidos, pagamento, cupons, admin, variantes, personalizações, avaliações, wishlist, SEO, API v1, segurança, performance, observabilidade). O sistema **já é um marketplace no código**: `Seller`, painel do vendedor, `SellerOrder` e split de comissão existem e estão testados (Fase 22 em andamento, Fase 23 concluída no código).

Frentes abertas:

- **Fase 20, Etapa B** — `Gateways::MercadoPago` existe e só é selecionado com `PAYMENT_GATEWAY=mercado_pago`; em produção, configuração ausente ou gateway fake falha explicitamente. PIX vencido, idempotência por tentativa e confirmação via webhook estão implementados. Os testes automatizados (Minitest/RSpec) stubam HTTP e cobrem o contrato do adapter, nunca a API real — mas **a integração já rodou contra o sandbox de verdade**: passou por um 500 opaco e um 403 investigados a fundo com o suporte do Mercado Pago (ver `docs/payments.md`, "Cartão de crédito (Fase 24)"), e desde 2026-09-22 PIX e cartão completam o checkout sem erro, local e em produção. **A aprovação final de pagamento ponta a ponta já foi observada em 2026-09-26** (PR #154): o payload de cartão não enviava `payment_method_id`, campo mínimo obrigatório do Mercado Pago — corrigido em `Gateways::MercadoPago#authorize_credit_card`, propagado desde o Card Payment Brick. Pedido de teste chegou a `paid`/`confirmed` com split e comissão de 15% conferidos exatos. Fase 24 concluída; não há mais bloqueio de pagamento por cartão.
- **Fase 22** — vínculo OAuth do vendedor com o Mercado Pago implementado (o provedor faz o KYC, a EloShop não coleta documentos, tokens ficam cifrados) **e já validado ponta a ponta**: a aplicação Marketplace está configurada (`MERCADO_PAGO_MARKETPLACE_*` e `PAYMENT_GATEWAY=mercado_pago` definidos na Railway) e há vendedores conectados em produção, entre eles um de sandbox (`Ateliê do Mercado Pago`, login `uriel@artesao.com.br`). Ele **reconectou em 2026-09-12 e já tem `Seller#mercado_pago_public_key` gravada** (conferido no banco em 2026-09-13), sendo o único dos cinco `Seller` que oferece cartão no checkout — testar cartão no sandbox não exige nenhum passo preparatório. Os demais conectaram antes da Fase 24 e precisariam reconectar; o PIX não depende disso. Esse estado vive no banco de produção e muda por fora do repositório: confira o `Seller` via `railway ssh` antes de repetir o que está escrito aqui. `/admin/contas-de-teste-mercado-pago` é um cofre **opcional** de credenciais TESTUSER e está vazio — isso **não** indica ausência de conta de teste, e já induziu a erro.
- **Frete** — a Etapa 1 do Melhor Envio existe, mas o WAF do provedor bloqueia o IP de saída da Railway com `E-WAF-0003`. Para o MVP, cada produto pode configurar entrega nacional por preço/prazo fixos por unidade e retirada gratuita; sem configuração, `Shipping::Calculator` preserva a tabela interna (R$ 15 base + R$ 5/kg). Etiqueta, rastreio e frete grátis promocional seguem como decisões posteriores — ver ADR 005 e `docs/shipping.md`.
...
**Login de `User` tem duas portas, mesma credencial.** `SessionsController` (`/session`) é o login da administração da plataforma; `SellerPortal::SessionsController` (`/painel/entrar`) é o login do vendedor — ambos autenticam contra a mesma tabela `User`/`Session`, mas cada um só aceita o papel correspondente (`SellerPortal::SessionsController#create` rejeita um admin mesmo com senha correta, com a mesma mensagem genérica de erro, para não revelar papel/existência da conta). `SellerPortal::BaseController` redireciona não autenticado para `seller_login_path`, nunca para `new_session_path`. O menu público (`_header.html.erb`) dá **um destino por papel de `User`**: o admin vê "Administração" (`/admin`), o vendedor vê o painel do ateliê, e quem não é nenhum dos dois vê o login/cadastro de artesão — o admin nunca vê convite ou login de ateliê, porque não é artesão. Esconder o link da administração não protegia nada (quem protege é o `require_admin!`) e só deixava o admin sem caminho pela interface. O topo também mostra a identidade da sessão de `User` (papel, quem está logado e um "sair" próprio de cada porta: `session_path` para o admin, `seller_logout_path` para o vendedor), porque `User` e `Customer` são sessões independentes que podem estar ativas ao mesmo tempo no mesmo navegador — sem esse sinal, quem estava logado como admin navegava a loja sem saber. `ApplicationHelper#user_session_identity` é a fonte única desses rótulos/destinos.
...
**Produção usa 4 bancos dentro da mesma instância Postgres.** `config/database.yml` faz parse manual da `DATABASE_URL` da Railway para derivar `<nome>`, `<nome>_cache`, `<nome>_queue`, `<nome>_cable` (Solid Cache/Queue/Cable) — usar `url:` nos quatro papéis faz `db:prepare` pular o schema de três deles. Deploy é via Dockerfile + Railway, não Kamal (`config/deploy.yml` existe mas não é usado). A configuração da infra vive em `.railway/railway.ts` (Infrastructure as Code) e está gravada no serviço; o `railway.json` foi removido. `railway config plan` mostra drift sem alterar nada, mas exige `npm install railway` na raiz — o projeto não tem toolchain Node, e `node_modules/` é gitignored. **Push no `main` deploya automaticamente**: o serviço `eloshop-web` está conectado ao repo no GitHub com "Wait for CI" ligado (`DeploymentTrigger.checkSuites`), então a Railway só builda depois que os check suites passam — um CI quebrado bloqueia todo deploy. Essa configuração é estado do lado da Railway, não está versionada — e **já se perdeu uma vez em silêncio**: reconectar a fonte do GitHub recria o trigger com `checkSuites` no default `false`. Não deduza que a proteção está valendo porque o build começou (um build bloqueado simplesmente não aparece); confira com a query `deploymentTriggers` ou compare horários — o deploy tem de começar depois de o CI concluir. Ver `docs/architecture.md`, "Deploy automático a partir do GitHub". Active Storage grava no volume persistente apontado por `RAILS_STORAGE_PATH`; sem ele, imagens somem a cada redeploy. Detalhes em `docs/architecture.md`, seção "Deploy".

**Front-end é Hotwire + importmap + Tailwind**, sem build de JS. A CSP é restritiva (`script_src :self` com nonce) — nada de scripts inline sem nonce nem CDN externo. Única exceção: os domínios exatos do Card Payment Brick do Mercado Pago, cada um só na diretiva que o usa (`secure-fields.mercadopago.com` é iframe, então vive em `frame_src` e **não** em `script_src`). `style_src` usa `unsafe_inline` porque o SDK injeta `<style>` em runtime sem nonce — por isso `style-src` saiu de `content_security_policy_nonce_directives`, já que o browser ignora `unsafe-inline` na presença de nonce. Nunca reintroduzir curinga de subdomínio: `spec/requests/security_headers_spec.rb` falha se voltar.
...
- frete → docs/shipping.md + docs/checkout.md + docs/decisions/005-shipping-provider.md
