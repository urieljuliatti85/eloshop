...
class NotifySellerOfDeliveryJobTest < ActiveJob::TestCase
  ...
  test "envia para o usuário do ateliê" do
    ...
    assert_emails 1 do
      NotifySellerOfDeliveryJob.perform_now(seller_order)
    end
  ...
  test "não envia nem levanta erro quando o ateliê não tem usuário" do
    ...
    assert_no_emails do
      assert_nothing_raised { NotifySellerOfDeliveryJob.perform_now(seller_order) }
    end
  end
end
