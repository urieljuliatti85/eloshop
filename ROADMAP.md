...
* [x] Cliente consegue pagar com cartão de crédito parcelado, vendo as opções de parcela calculadas pelo Bricks — implementado; **não verificado num browser real**. O fluxo PIX foi validado ponta a ponta via HTTP direto; o Brick em si exige vendedor com Public Key real, que segue sendo a única pendência (a falta de browser na sessão que implementou deixou de ser obstáculo — ver a pendência abaixo)
...
* **Verificação visual do Brick num browser real** — ainda não feita, mas **o motivo registrado antes estava errado**: não é falta de browser. O projeto já dirige um browser de verdade (Capybara + Selenium nos system tests, e a skill `run-mobile-responsive-check` via CDP), e as sessões do Claude Code na web trazem Chromium pré-instalado em `/opt/pw-browsers` (verificado em 2026-09-09). O fluxo PIX foi validado ponta a ponta via requisições HTTP diretas (curl com cookies), mas o Card Payment Brick em si — carregamento do SDK, iframes, captura do token, parcelas exibidas — nunca foi visto renderizado. A **única dependência real é a Public Key**: nenhum seed tem, e nenhum dos dois vendedores conectados em produção gravou a coluna, porque ambos conectaram antes da Fase 24 (um clique de reconexão no painel do artesão resolve — ver o parágrafo de 2026-09-08).
...
`Fase 24 (cartão de crédito): **tudo que depende do código da EloShop está feito e observado num navegador**; o que falta é externo. A verificação visual de 2026-09-12 rendeu cinco defeitos reais, todos corrigidos (#76 a #80) e nenhum detectável pelas suítes. O Brick renderiza, tokeniza o cartão no navegador e o backend chama o gateway; a falha é tratada com clareza para o cliente, com caminho de volta. **O bloqueio é o HTTP 500 do Mercado Pago em `/v1/payments`**, com `error: internal_error` e `cause: []` — resposta opaca, reproduzida com cartão de teste oficial e 1 parcela, e idêntica à que o PIX levou em 2026-09-09 na mesma conta. **Próximo passo: abrir chamado com o Mercado Pago**, com a evidência do parágrafo acima (conta `143360137`, endpoint, payload aceito, dois meios de pagamento, duas datas). Enquanto isso não se resolver, o split e a comissão de 15% seguem **sem validação ponta a ponta** — são a única parte do §34 que nenhum caminho local exercita.
