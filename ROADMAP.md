...
`Fechar as duas pendências restantes da Fase 24 (cartão de crédito). **Apertar a CSP saiu da lista: foi feito em 2026-09-07** (commit `eae9ba7`), trocando os curingas pelos domínios exatos, cada um só na diretiva que o usa, com `spec/requests/security_headers_spec.rb` travando as invariantes. Restam: verificar visualmente o Card Payment Brick num browser real com um vendedor tendo Public Key configurada — o que **não depende mais de ferramenta**, já que Chromium está disponível nas sessões da web e o projeto já dirige browser nos system tests, e sim de reconectar um vendedor para gravar a `mercado_pago_public_key` (um clique no painel do artesão); e testar o fluxo de cartão contra o sandbox do Mercado Pago, para o qual a conta TESTUSER existente **já serve**, porque ela lista cartão entre os métodos disponíveis (o que faltou nela foi PIX). Essa mesma reconexão exercita a CSP num browser real de tabela.
