...
>
> **Confirmado em produção no mesmo dia, via `railway ssh` — e o JIT NÃO acontece lá.** `jit=on` e `jit_above_cost=100000` são iguais aos de dev, mas o plano da mesma query custa **64.496** (contra 529.762 local), fica **abaixo do limiar** e não gera bloco `JIT`: `Execution Time` de **0,417 ms**. A PDP inteira responde em **13,22 ms de banco em 12 queries**, com a mais cara em 3,74 ms — nada perto dos 83,75 ms que abriram a investigação. `includes` e `preload` empatam (26/15 ms contra 12/17 ms), então **não há correção a aplicar**.
>
> Por que o custo estimado difere 8x entre bancos com 10 e 12 produtos: o plano depende das **estatísticas** do planner (`ANALYZE`), não só da contagem de linhas. O banco de dev acumulou seeds e recriações sem `ANALYZE` recente, e o planner passou a superestimar — 244.811 linhas onde a tabela tem dezenas. **Medição local de plano não substitui a de produção**, e este é o quarto episódio da Fase 17 em que uma medição imprecisa gerou conclusão errada.
>
> O que sobra: os 83,75 ms de `db_runtime` vieram da amostra de tráfego real de 2026-09-01. Com a PDP hoje em 13 ms de banco, ou a medição de setembro incluía outra coisa (fila de conexão, cold start), ou o quadro mudou desde então. **Não há gargalo a corrigir agora** — reabrir só se o tráfego real voltar a mostrar a PDP lenta.
...
`Duas decisões do negócio, ambas fora do escopo já fechado da Fase 17: (a) a PDP — ENCERRADO em 2026-09-06 sem correção. A medição em produção via `railway ssh` mostrou a PDP em 13,22 ms de banco (12 queries, a mais cara em 3,74 ms), sem bloco JIT: o plano da query de relacionados custa 64.496, abaixo do jit_above_cost de 100.000. `includes` e `preload` empatam. O JIT de 1520 ms que apareceu no diagnóstico local era artefato de estatísticas desatualizadas do planner no banco de dev, que superestimava 244.811 linhas. Os 83,75 ms da amostra de 2026-09-01 não se reproduzem hoje; reabrir só se o tráfego real voltar a mostrar a PDP lenta. Ver a nota "Diagnóstico local em 2026-09-06" na Fase 17; (b) ajustar jit_above_cost (ou jit=off) no PostgreSQL da Railway, que protegeria qualquer query futura com plano superestimado, mas é mudança de infraestrutura com efeito global. Seguem bloqueadas por dependência externa: criar/configurar a aplicação Marketplace de testes do Mercado Pago, registrar a callback /painel/mercado-pago/callback, definir MERCADO_PAGO_MARKETPLACE_SANDBOX=true e validar o OAuth com uma conta TESTUSER Vendedor (Fase 22), e o teste PIX ponta a ponta no sandbox (Fase 20, Etapa B).`
