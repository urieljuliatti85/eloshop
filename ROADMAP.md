...
No frete (ADR 005), a Etapa 1 está implementada e a próxima dependência é externa: criar a aplicação OAuth no Melhor Envio, definir MELHOR_ENVIO_CLIENT_ID/CLIENT_SECRET/REDIRECT_URI, registrar a callback /painel/melhor-envio/callback e validar a cotação contra o sandbox (que tem R$ 10.000 de saldo fictício). As Etapas 2 (etiqueta) e 3 (webhook de rastreio) continuam travadas pelas decisões de negócio do ADR 005 — quem paga a etiqueta, se existe frete grátis, e quem absorve a diferença quando o frete real sai mais caro que o cobrado.
