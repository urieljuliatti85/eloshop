...
* **Cartão exercitado contra a API real em 2026-09-12, sem concluir** — deixou de ser "nunca testado": a chamada a `/v1/payments` acontece de verdade e falha com HTTP 500 `internal_error`/`cause: []` do provedor (ver o parágrafo do dia). Os testes seguem stubando HTTP e cobrindo o contrato do adapter, não a API real; o desfecho aprovado continua sem observação — e com ele o split e a comissão de 15%.
