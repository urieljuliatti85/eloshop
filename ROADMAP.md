...
* [x] Medição sob tráfego real em produção realizada (2026-09-01, segunda amostra abaixo); apontou o catálogo como gargalo, corrigido e validado em produção na mesma data
...
**Achado colateral, não corrigido**: a PDP (`ProductsController#show`) usa o mesmo `includes` + `with_attached_main_image` em `@related_products` e cai no mesmo JIT — o plano tem bloco `JIT` e a query custa 68–86 ms localmente (produção mediu a PDP em 146 ms). O impacto é uma ordem de grandeza menor porque `related_products` traz no máximo 4 itens, e a correção seria a mesma troca por `preload`. Não foi aplicada aqui para manter o diff no escopo do gargalo medido; decidir se entra nesta fase ou vira tarefa própria. Confirmado em produção na medição pós-deploy: a PDP aparece com `db_runtime` de 83,75 ms contra ~10 ms do catálogo já corrigido, coerente com a previsão.

> **Contestado em 2026-09-05 — não aplicar a troca sem medir de novo.** Uma remedição local do `related_products`, com `EXPLAIN (ANALYZE)` dentro de `ActiveRecord::Base.uncached`, **não reproduziu o bloco `JIT`**. A forma da query confere com o descrito acima (14 JOINs, 129 colunas), mas o custo *estimado* do plano é **128**, contra um `jit_above_cost` de **100.000** — 780x abaixo do limiar, e o plano estima 75 linhas, não centenas de milhares. Na mesma medição, `preload` ficou ~70x mais lento que `includes` (29 ms em 7 queries contra 0,41 ms em uma), porque sem JIT a query única é barata e o `preload` só acrescenta idas ao banco.
>
> A diferença em relação ao catálogo é estrutural, não de escala: `related_products` filtra por vendedor e tem `LIMIT 4`, então a estimativa do plano cresce com o catálogo do *vendedor*, não com a tabela inteira — o catálogo varria tudo e estimava ~985.759 linhas. "Mesmo padrão de código" não implicou "mesma causa".
>
...
>
> **Correção aplicada em 2026-09-06** (`app/controllers/products_controller.rb#show`): `related_products` trocou `includes` + `with_attached_main_image` por `preload`, mesmo padrão do catálogo. Uma terceira remedição local (10 produtos, 1 anexo `main_image` cada) **não reproduziu o bloco JIT** desta vez: `EXPLAIN (ANALYZE)` mostrou custo real de ~108, muito abaixo do `jit_above_cost` de 100.000, com `includes` e `preload` praticamente empatados (67 ms vs 55 ms de banco). A terceira medição consecutiva com resultado diferente das duas anteriores confirma que o dataset local é volátil demais para decidir isso sozinho — a causa raiz (o mecanismo do JIT com `with_attached_main_image`) já foi isolada e é a mesma do catálogo, então a correção foi aplicada por ser estruturalmente segura (comportamento idêntico, suíte completa verde, sem regressão possível), não porque a medição local a confirmou desta vez.
>
> **Validação em produção (2026-09-06) — CONFIRMADA.** PR #60 mergeado, deploy `8f341ea` com `SUCCESS`. Três requisições à PDP (`/artesaos/eloshop/produtos/luminaria-de-mesa-em-ceramica`) nos eventos `action_controller.request_completed`:
>
> | | antes | depois |
> | --- | --- | --- |
> | `db_runtime` | 83,75 ms | **17,8 / 10,1 / 10,0 ms** |
> | `duration_ms` | — | **50,1 / 36,0 / 31,0 ms** |
> | `queries_count` | — | 25 (2 cached) |
>
> Queda de ~5–8x no tempo de banco, em linha com o esperado se o JIT era o custo — o mesmo desfecho do catálogo. A medição local não ter reproduzido o JIT foi artefato do dataset pequeno, não sinal de que a hipótese estava errada. Ressalva de amostra: 3 requisições deliberadas, não tráfego orgânico.

### Validação em produção (2026-09-01)

A correção foi mergeada (PR #30, commit `6e41b0e`), o deploy automático fechou com `SUCCESS` e os eventos `action_controller.request_completed` do deploy novo confirmam o resultado:

| | antes | depois |
| --- | --- | --- |
| `duration_ms` | 1012 ms (p50) / 1497 ms (p95) | **56,7 / 40,3 / 42,5 ms** |
| `db_runtime` | 959–1314 ms | **19,6 / 9,1 / 9,9 ms** |
| `queries_count` | 14 | 17 |

Queda de ~24x na duração e ~100x no tempo de banco. O `db_runtime` de ~10 ms é a confirmação direta de que o JIT deixou de ser compilado — era ele que consumia os ~1240 ms. A contagem sobe de 14 para 17 queries por construção: `preload` troca um JOIN monolítico por queries pequenas, e o critério da fase sempre foi sobre crescimento, não sobre total absoluto.

Ressalva de amostra: são 3 requisições deliberadas, não tráfego orgânico. Bastam para validar a correção (o `db_runtime` é inequívoco e a causa raiz está isolada), mas o p50/p95 sob carga real merece uma releitura dos logs quando houver volume.

**Decisão separada, ainda não aplicada**: ajustar `jit_above_cost` (ou `jit=off`) no PostgreSQL da Railway protegeria qualquer query futura com plano superestimado, mas é mudança de infraestrutura com efeito global e depende de decisão explícita.
...
Status: `[~]` em andamento — código completo, verificação visual do Brick e CSP concluídas; fluxo de PIX e cartão completa sem erro local e em produção desde 2026-09-22, mas ainda falta observar uma aprovação final de pagamento ponta a ponta. Ver "Estado atual" no fim do roadmap.

### Objetivo

Oferecer cartão de crédito como segundo meio de pagamento, ao lado do PIX já existente (Fase 20, Etapa B), usando Checkout Bricks (Card Payment Brick) do Mercado Pago para tokenização no front. A decisão original de começar só por PIX (`docs/payments.md`, Fase 20 Etapa B) é revisitada aqui, não descartada retroativamente — PIX segue funcionando sem regressão.

Decisões de negócio já tomadas para esta fase:

* Tokenização via Checkout Bricks (componente pronto do Mercado Pago), não formulário próprio
* Autorização e captura em uma etapa só, igual ao PIX hoje — sem captura em duas etapas nesta primeira versão
* Parcelamento habilitado desde o início, usando as opções calculadas pelo próprio Brick
* Juro do parcelamento é repassado ao cliente (taxa do emissor); a loja/vendedor não assume custo extra de "parcelamento sem juros" — a fórmula de comissão de 15% sobre produtos após descontos, sem frete (Fase 23), não muda

### Diferença fundamental do fluxo PIX

PIX é assíncrono: `authorize` cria a cobrança e o status só muda depois via webhook, quando o cliente paga o QR code. Cartão é síncrono: o Mercado Pago aprova ou recusa **na própria resposta HTTP** do `authorize`. O checkout precisa ramificar entre a tela de PIX (como hoje) e o resultado imediato do cartão — aprovado segue para confirmação, recusado permite nova tentativa sem debitar estoque.

### Funcionalidades

* Nova opção de meio de pagamento no checkout, ao lado do PIX
* Card Payment Brick embutido na etapa de pagamento, tokenizando os dados do cartão no navegador — nenhum dado sensível (número, CVV) trafega pelo backend
* Exibição das opções de parcela calculadas pelo Brick
* Resultado síncrono de aprovação/recusa tratado no mesmo request do checkout
* Split de comissão (`application_fee`) aplicado igual ao PIX

### Modelos envolvidos

* `Payment` — novos campos `payment_method` (`pix` / `credit_card`) e `installments`; possivelmente `card_last_four`/`card_brand` só para exibição (nunca dado sensível completo)
* `Gateways::Intent` — já neutro a meio de pagamento (ver comentário em `app/services/gateways/intent.rb`)

### Migrations necessárias

Adicionar `payment_method` (string) e `installments` (integer, default 1) a `payments`; campos de exibição do cartão (`card_last_four`, `card_brand`) se necessário para o admin/cliente identificarem a tentativa.

### Arquivos principais a alterar

* `app/services/gateways/mercado_pago.rb` — `authorize` aceita `payment_method_id:`/`card_token:`/`installments:` e monta o corpo da requisição conforme o meio
* `app/services/gateways/fake_gateway.rb` — simular aprovação/recusa de cartão em dev/test
* `app/services/payments/authorize.rb` — repassar os novos parâmetros ao gateway
* Controller da etapa de pagamento do checkout — ramificar entre PIX e resultado síncrono do cartão
* CSP (`config/initializers/content_security_policy.rb` ou equivalente) — liberar o domínio do SDK JS do Mercado Pago (`sdk.mercadopago.com`) e os domínios que o Brick usa para tokenizar

### Testes

* `Gateways::MercadoPago`: contrato do adapter para cartão — aprovação, recusa, erro de gateway, parcelas inválidas
* `Payments::Authorize` com `payment_method: "credit_card"`
* Request specs do checkout cobrindo o novo meio de pagamento
* Edge cases obrigatórios (CLAUDE.md §46): cartão recusado, token expirado/inválido, timeout na tokenização, tentativa duplicada (double-submit do Brick)
* Regressão: fluxo PIX continua funcionando sem alteração de comportamento

### Riscos

* Primeira vez que o app carrega script de CDN externo — validar que a CSP não abre brecha além do necessário
* Mesmo com Bricks, a loja entra em escopo leve de PCI-DSS (SAQ A) — confirmar que isso não é bloqueio de negócio antes de habilitar em produção
* Sandbox nunca testado para cartão, mesma situação já registrada para PIX (Fase 20, Etapa B) — falta credencial de teste
* Chargebacks/disputas de cartão têm prazo e fluxo diferentes de PIX; fora do escopo desta fase, registrar como débito técnico se não for endereçado

### Critérios de aceite

* [x] Cliente consegue pagar com cartão de crédito parcelado, vendo as opções de parcela calculadas pelo Bricks — implementado e **verificado num browser real em 2026-09-12**: o Brick renderiza, tokeniza o cartão e o backend chama o gateway. A dependência da Public Key **está resolvida** — o `Ateliê do Mercado Pago` reconectou e a chave está gravada (conferido no banco de produção em 2026-09-13). O que impede a conclusão é externo: o HTTP 500 do Mercado Pago em `/v1/payments`
...
* ~~**Apertar a CSP**~~ — **CONCLUÍDO em 2026-09-07** (commit `eae9ba7`). A suposição registrada aqui era que faltaria fonte para os domínios internos do Brick e que só o console do browser resolveria; na prática a lista saiu da leitura do bundle servido em `https://sdk.mercadopago.com/js/v2`, onde `cacheUrl`/`sourceUrl`/`assetsBaseUrl` e as URLs de XHR estão explícitos no código. Os curingas foram trocados pelos seis domínios exatos, cada um só na diretiva que o usa (`secure-fields.mercadopago.com` em `frame-src`, nunca em `script-src`), o TODO saiu de `config/initializers/content_security_policy.rb` e `spec/requests/security_headers_spec.rb` trava as invariantes. Exercitar num browser real segue junto da verificação do Brick, abaixo.
* ~~**Verificação visual do Brick num browser real**~~ — **FEITA em 2026-09-12** (ver o parágrafo do dia); a dependência da Public Key foi resolvida por reconexão e está confirmada no banco desde 2026-09-13. O registro abaixo é histórico. O motivo registrado antes **estava errado**: não era falta de browser. O projeto já dirige um browser de verdade (Capybara + Selenium nos system tests, e a skill `run-mobile-responsive-check` via CDP), e as sessões do Claude Code na web trazem Chromium pré-instalado em `/opt/pw-browsers` (verificado em 2026-09-09). O fluxo PIX foi validado ponta a ponta via requisições HTTP diretas (curl com cookies), mas o Card Payment Brick em si — carregamento do SDK, iframes, captura do token, parcelas exibidas — nunca foi visto renderizado. A **única dependência real era a Public Key**: nenhum seed tem, e nenhum dos vendedores conectados em produção havia gravado a coluna, porque conectaram antes da Fase 24 — resolvido pela reconexão do `Ateliê do Mercado Pago` (ver o parágrafo de 2026-09-13).
* ~~**Cartão exercitado contra a API real em 2026-09-12, sem concluir**~~ — o HTTP 500 `internal_error`/`cause: []` do provedor não voltou a ocorrer: em 2026-09-22 o usuário confirmou que PIX e cartão completam o fluxo de checkout sem erro tanto localmente quanto em produção (`https://eloshop-web-production.up.railway.app/`). **Ressalva importante**: essa confirmação cobre o fluxo/tela (Brick carrega, tokeniza, o backend chama o gateway e retorna sem erro) — ainda **não** foi observada uma aprovação final de pagamento ponta a ponta (pedido pago de fato, com status confirmado). O desfecho aprovado, e com ele o split e a comissão de 15%, seguem sem essa observação. Os testes seguem stubando HTTP e cobrindo o contrato do adapter, não a API real.
...
Fase 20 (Etapa B — gateway Mercado Pago), Fase 23 (split de comissão).
...
PIX E CARTÃO SEM ERRO NO FLUXO COMPLETO, LOCAL E EM PRODUÇÃO, confirmado pelo usuário em 2026-09-22 — o HTTP 500 `internal_error` do Mercado Pago (parágrafos "CARTÃO EXERCITADO PONTA A PONTA EM PRODUÇÃO" e seguintes, 2026-09-12/13) não voltou a ocorrer: o checkout completa PIX e cartão sem erro tanto localmente quanto em `https://eloshop-web-production.up.railway.app/`. **Ressalva (resolvida em 2026-09-26, ver parágrafo do dia)**: a confirmação cobria o fluxo (Brick carrega, tokeniza, o backend chama o gateway e responde sem erro), mas ainda faltava observar uma aprovação final de pagamento ponta a ponta.

APROVAÇÃO FINAL DE CARTÃO OBSERVADA PONTA A PONTA EM PRODUÇÃO, em 2026-09-26 — a última ressalva da Fase 24 foi resolvida. Causa raiz: o payload de `POST /v1/payments` para cartão não enviava `payment_method_id`, campo mínimo obrigatório do Mercado Pago (junto com `token`/`transaction_amount`/`installments`/`payer.email`) — sem ele o provedor respondia com 500 opaco em vez de rejeitar com mensagem clara, reincidência dos WCS-50393/WCS-51962. Corrigido em `Gateways::MercadoPago#authorize_credit_card`: `payment_method_id` passou a ser obrigatório (levanta `ArgumentError` se ausente) e propagado desde o Card Payment Brick no navegador (`cardFormData.payment_method_id`) até o gateway; `issuer_id` segue o mesmo caminho, mas é opcional — omitido do payload quando o Brick não o devolve, nunca enviado como `nil`. PR #154, mesclado e deployado em produção no mesmo dia. Teste real com cartão de teste Mastercard (`APRO`) no `Ateliê do Mercado Pago`: **Payment #42** chegou a `status=paid` (gateway `mercado_pago`, `external_id=1328280608`), **Order #53** a `status=confirmed`, e o **SellerOrder** correspondente fechou a aritmética exata do §34 — `subtotal_cents=100`, `platform_fee_cents=15` (15% do subtotal, sem frete), `seller_amount_cents=85`. Fase 24 concluída: não resta nenhuma pendência de código nem de observação para o pagamento via cartão.

FASE 24 em andamento (2026-09-07): cartão de crédito via Checkout Bricks implementado ao lado do PIX. `Gateways::MercadoPago#authorize` ramifica entre PIX (assíncrono) e cartão (síncrono); `Payments::Authorize` confirma o pedido sincronamente reaproveitando `Payments::ProcessWebhook` com um `event_id` sintético. `Seller` ganhou `mercado_pago_public_key` (não cifrado, capturado do OAuth) — vendedores conectados antes desta fase precisam reconectar para a opção de cartão aparecer. O checkout ganhou uma tela de escolha de meio de pagamento: `GET .../payment/new` deixou de autorizar automaticamente (mudança de comportamento do fluxo PIX, que agora exige um clique a mais); `POST .../payment` autoriza o meio escolhido. Pipeline local verde: 485 Minitest, 383 RSpec, rubocop e brakeman sem achados. O fluxo PIX foi validado ponta a ponta via requisições HTTP diretas (login → carrinho → checkout → escolha → aprovação simulada → pedido confirmado). Pendências: a CSP **foi apertada em 2026-09-07**, trocando `*.mercadopago.com`/`*.mlstatic.com` pelos domínios exatos, cada um só na diretiva que o usa — a lista veio da leitura do bundle servido em `sdk.mercadopago.com/js/v2` (a doc oficial dos Bricks não trata de CSP), onde `cacheUrl`/`sourceUrl`/`assetsBaseUrl` e as URLs de XHR estão explícitos; `secure-fields.mercadopago.com` ficou só em `frame-src`, nunca em `script-src`. Achado no caminho: o SDK injeta `<style>` em runtime **sem nonce**, então `style-src` precisou de `unsafe-inline` e saiu de `content_security_policy_nonce_directives` (o browser ignora `unsafe-inline` quando há nonce na diretiva) — o curinga anterior nunca teria resolvido isso, porque o problema é inline e não domínio. `spec/requests/security_headers_spec.rb` passou a travar as invariantes (nenhum curinga, `script-src` com nonce e sem `unsafe-inline`, cada domínio na sua diretiva). A CSP **foi exercitada num browser real em 2026-09-12** e a lista de domínios estava correta; o Card Payment Brick foi verificado visualmente no mesmo dia e a Public Key do vendedor de sandbox está gravada desde então (confirmado no banco em 2026-09-13); o que resta é o HTTP 500 do provedor em `/v1/payments`, dependência externa. Detalhes em `docs/payments.md`, seção "Cartão de crédito (Fase 24)".

AVISOS DO PEDIDO CONFIRMADO (fan-out) implementados em 2026-09-07. Até aqui um cliente comprava e **não recebia e-mail nenhum**, e o artesão não ficava sabendo que tinha vendido: `Order#confirm!` só mudava o status. `Payments::ProcessWebhook` passa a enfileirar três jobs quando — e somente quando — o evento confirma o pedido: `SendOrderConfirmationJob` (cliente), `NotifySellerOfOrderJob` (artesão, com a comissão discriminada) e `RecordOrderAnalyticsJob` (evento `order.confirmed` no log estruturado da Fase 19). Um job por aviso, para que a falha de um não impeça os outros. **A idempotência não exigiu coluna nova**: `ALLOWED_STATUS_TRANSITIONS` não permite `confirmed → confirmed`, então `apply_status!` devolve `true` só quando aquele evento foi o que confirmou — webhook repetido, retry com `event_id` novo e a confirmação síncrona do cartão convergem no mesmo ponto sem duplicar. O enfileiramento é fora da transação, porque a fila vive em outro banco e um worker poderia ler o job antes do commit. Achado colateral corrigido: `ApplicationMailer` não incluía `ApplicationHelper`, então `format_price` não existia nos templates — o primeiro teste de entrega real pegou isso. Pipeline local verde: 529 Minitest, 409 RSpec, rubocop e brakeman sem achados; fluxo exercitado no app real, com os dois e-mails e o evento conferidos (comissão de 15% batendo: R$ 89,90 − R$ 13,49 + R$ 20,00 de frete = R$ 96,41 ao artesão). Contexto: a proposta original era usar Kafka para esse fan-out; a decisão foi Active Job + Solid Queue, que já estão em produção — Kafka acrescentaria um broker entre um produtor que já funcionava e consumidores que não existiam, e o passo de estoque nem é assíncrono (é debitado na transação do checkout, com `lock!`, para não vender duas vezes a mesma peça única). Falta e-mail de pedido enviado/entregue, que depende da Etapa 2 do frete (ADR 005).

PUBLIC KEY JÁ GRAVADA E DOIS VENDEDORES NÃO DOCUMENTADOS, apurado em 2026-09-13 — **o roadmap voltou a descrever como pendente algo já feito**, segunda ocorrência do padrão registrado em 2026-09-08. Consultado o banco de produção via `railway ssh`, o estado real dos `Seller` é: `EloShop` (`live_mode: true`, sem Public Key), `Ateliê da Ana` (`live_mode: true`, sem Public Key), `Rock Store` (`live_mode: false`, sem Public Key), `House of Horror` (`live_mode: false`, sem Public Key) e **`Ateliê do Mercado Pago` (`live_mode: false`, COM Public Key)**. A reconexão do vendedor de sandbox aconteceu em 2026-09-12, às 17:22, e é justamente o que permitiu a verificação visual do Brick daquele dia — o roadmap seguia afirmando que "nenhum dos dois vendedores conectados gravou a coluna". **Consequência prática**: o teste de cartão no sandbox não exige mais nenhum passo preparatório; basta comprar um produto do `Ateliê do Mercado Pago` e escolher cartão, porque é o único dos cinco que oferece o meio (`mercado_pago_card_payments_available?` exige a Public Key). Achado colateral: `Rock Store` e `House of Horror` **não constam em nenhuma documentação** — origem não apurada, ambos sem conexão utilizável para cartão. Erro de método registrado: a documentação descreve o estado do banco de produção, que muda por fora do repositório — depois de qualquer reconexão ou conexão de vendedor, conferir o `Seller` antes de repetir o que está escrito.

PIX NÃO É TESTÁVEL NO SANDBOX, apurado em 2026-09-09 — **isto invalida o plano que este roadmap descrevia**. O cliente relatou "o pedido foi salvo, mas o pagamento está temporariamente indisponível" e os logs de produção mostraram duas falhas no dia (09:02 e 09:23 UTC, pedido #26): `POST /v1/payments` devolvendo HTTP 500, agora com `error=internal_server_error` e `cause` vazio — sinal diferente do `user_allowed_only_in_test` de 2026-09-08. Foi PIX, não cartão: a UI de cartão só é renderizada com `mercado_pago_card_payments_available?` e não houve nenhuma reconexão, então a Public Key continua ausente. A causa registrada em 2026-09-08 ("falta cadastrar chave PIX na conta TESTUSER") **estava errada**: a documentação oficial afirma que pagamentos com PIX não podem ser feitos com credenciais de teste, e a página de teste de integração recomenda usar outro meio para validar o QR Code. A observação de `/v1/payment_methods` segue correta — PIX não aparece porque não existe para credencial de teste, não por configuração faltando. Consequências: o teste de PIX ponta a ponta **só existe em produção**, com conta real e valor baixo (decisão de negócio, envolve dinheiro real); o **cartão** é o meio a exercitar no sandbox, com os cartões de teste do provedor, e depende apenas da reconexão do vendedor. Erro de método registrado, terceira ocorrência do mesmo padrão: a observação era boa e a causa foi inferida dela sem consultar a documentação — a regra passa a ser conferir a doc oficial antes de fechar causa em limitação de ambiente. Corrigido no caminho um comentário enganoso em `PaymentsController#create`, que afirmava que a tentativa é marcada `failed` no rescue: ela fica `processing` de propósito (preserva a chave de idempotência), e quem oferece o "Tentar novamente" é `Payment#stalled?` depois de 2 minutos — o comentário descrevia a correção que foi revertida em 2026-09-08.

CARTÃO EXERCITADO PONTA A PONTA EM PRODUÇÃO, em 2026-09-12 — **toda a cadeia da EloShop funciona; o bloqueio que resta é do provedor**. Com o vendedor reconectado e as quatro correções do dia no ar, o Card Payment Brick renderizou os campos, tokenizou o cartão no navegador (o log mostra `card_token=[FILTERED]`, o filtro de parâmetros protegendo o dado sensível conforme §43) e o backend chamou `/v1/payments`. **A criação da cobrança falhou com HTTP 500**, `error: internal_error`, **`cause: []`** — resposta genérica, sem indicação de campo inválido, credencial errada ou método não habilitado. Duas tentativas: pedido #34 às 18:17 UTC com `installments: 2`, e às 18:33 com `installments: 1` e o cartão de teste oficial (`5480 8328 0103 3311`, titular `APRO`, CPF `12345678909`) — mesmo resultado. Isso **descarta** payload malformado (viria `cause` com o campo), credencial (viria `unauthorized`), número de cartão e parcelamento. É o mesmo sintoma registrado em 2026-09-09, quando o PIX falhou com `internal_server_error` e `cause` vazio na mesma conta: mesma conta de teste (identificador `143360137`), mesmo endpoint, mesma resposta opaca, agora em dois meios de pagamento diferentes. O log `payment.mercado_pago_gateway_http_error`, tornado permanente em 2026-09-08 justamente para este caso, é o que permitiu ver o `cause` vazio — sem ele a exceção diria só "respondeu 500". **Próximo passo é externo**: abrir chamado com o Mercado Pago com essa evidência. Não deduzir a causa a partir do 500 — foi o erro de método cometido quatro vezes nesta integração, e `cause: []` é precisamente a ausência da informação que a confirmaria.

CINCO DEFEITOS ACHADOS NUM DIA DE VERIFICAÇÃO VISUAL, todos em 2026-09-12 e **nenhum detectável pelas suítes**, que passavam verdes em todos eles: (1) o botão de cartão fora do escopo do controller Stimulus, clique que não chegava a `showCard` (#76); (2) o `<script>` inline do antifraude bloqueado pela CSP, Brick preso no skeleton, resolvido com `deviceProfileCspNonce` sem afrouxar a política (#77); (3) `onReady` ausente nos callbacks, que fazia `cardPayment.js` recusar inicializar (#78); (4) `secure-fields.mercadopago.com` só em `frame-src`, faltando no `connect-src` que o SDK usa para XHR (#79); (5) o "Tentar novamente" apontando para a própria tela de falha, deixando o cliente em beco sem saída — fatal para cartão, cujo token do Brick é de uso único (#80). O denominador comum: **request spec vê o elemento no HTML e passa; as specs de CSP afirmam que o cabeçalho está bem formado, nunca que o SDK funciona sob ele**. Cada correção destravou a seguinte e revelou o próximo obstáculo, o que só um navegador podia mostrar. Erro de método repetido quatro vezes e registrado: atribuí o travamento do Brick ao vendedor local fabricado, hipótese derrubada quando o mesmo travamento se repetiu em produção com vendedor real — a regra que vale é ler o console antes de fechar causa.

VALIDADO DE QUEBRA, sem ter sido o alvo: o `Payment#stalled?` de 2026-09-08 foi observado funcionando pela primeira vez — passados os 2 minutos de `PROCESSING_STALE_AFTER`, a tela trocou "Preparando a cobrança" por "Não foi possível preparar a cobrança. Seu pedido continua salvo", com o caminho de volta. Tinha teste, não tinha observação. E o `[FILTERED]` no `card_token` confirma que o filtro de parâmetros protege o dado de cartão no caminho real, não só em teste.

SECURE-FIELDS FALTAVA NO `connect-src`, e o Brick esbarrou num 502 DO PROVEDOR, em 2026-09-12. Com o `onReady` no ar, o Brick passou da validação e chegou a montar os campos, revelando os dois últimos obstáculos. (1) **Nosso**: `secure-fields.mercadopago.com` estava só em `frame-src`, mas o SDK também faz XHR para ele (`fetchPage`) ao montar — o comentário do initializer registrava "só frame-src, nunca script-src", correto quanto a script e incompleto quanto a connect. Acrescentado ao `connect-src`; não afrouxa nada, é domínio já confiado para o iframe, agora na diretiva do outro uso. Coberto em `security_headers_spec.rb`, verificado falhando sem a mudança. (2) **Do Mercado Pago**: `GET https://api-static.mercadopago.com/secure-fields` responde **HTTP 502** — confirmado por `curl` fora do navegador, com e sem `Origin`, portanto não é CSP nem CORS nem a aplicação. O erro de CORS que aparece junto é consequência (resposta 502 não traz `Access-Control-Allow-Origin`), e o `TypeError: Cannot read properties of undefined (reading 'message')` é o SDK quebrando ao ler o erro que não veio. Enquanto esse endpoint estiver fora, o Brick não monta, independentemente do que fizermos.

Sequência do dia, cada correção destravando a seguinte: botão fora do escopo do controller Stimulus (#76) → script inline do antifraude bloqueado, resolvido com `deviceProfileCspNonce` (#77) → `onReady` ausente, que fazia o `cardPayment.js` recusar inicializar (#78) → `secure-fields` fora do `connect-src` → 502 do provedor. Os quatro primeiros eram nossos e estão corrigidos; o quinto não é. **Nenhum deles era detectável pelas suítes**: request spec vê o botão no HTML mas não clica, e as specs de CSP afirmam que o cabeçalho está bem formado, nunca que o SDK funciona sob ele. O método que achou todos foi o mesmo — abrir o console e ler, em vez de deduzir a causa de sinal indireto.

**Decisão pendente do negócio**: o antifraude (device profile) tenta carregar pixels de `www.mercadolibre.com` e `www.mercadolivre.com` que a CSP bloqueia (`img-src`), além de um `connect` a `/jms/lgz/background/etid`. O conteúdo desses pixels é fingerprinting do dispositivo — a carga em base64 traz lista de plugins, presença de webcam e tipos MIME suportados. Liberar autoriza essa coleta num domínio que hoje não está na política; não liberar deixa o antifraude degradado, com efeito desconhecido sobre recusa e chargeback. Não impede o pagamento, então segue em aberto. Uma terceira via existe: `advancedFraudPrevention: false` desliga a tentativa explicitamente, ao custo de abrir mão da proteção.

O BRICK ERA BLOQUEADO PELA CSP, e a causa registrada horas antes **estava errada**, corrigido em 2026-09-12. O Card Payment Brick parava no skeleton e nunca renderizava os campos. Atribuí isso ao vendedor local ser fabricado (Public Key real, `mercado_pago_user_id` e tokens inventados) — hipótese **derrubada pela observação**: com o `Ateliê do Mercado Pago` reconectado em produção às 17:22, vendedor real e tokens vindos do OAuth, o Brick travou exatamente igual. A causa real veio do console: `Executing inline script violates the following Content Security Policy directive 'script-src ...'`. O SDK injeta em runtime um `<script>` inline com o widget antifraude (device profile, buscado em `/devices/widgets`) e a CSP o bloqueia — o Brick fica preso e **o antifraude não roda**, o que valia para toda transação de cartão, não só em teste. Quarta ocorrência do mesmo erro de método: inferi a causa de um sinal indireto (o vendedor era falso, logo seria isso) em vez de ir ao console, que responde direto.

A correção **não mexe na CSP**: o SDK expõe `deviceProfileCspNonce`, opção pública que ele aplica no script injetado (`e && (n.nonce = e)`, e valida o tipo). A view passa `content_security_policy_nonce` ao controller Stimulus, que o repassa ao construtor. Descartadas: `unsafe-inline` em `script-src` (desligaria a proteção contra XSS na tela onde o cliente digita o cartão, e obrigaria a abrir mão do nonce inteiro, já que o browser ignora `unsafe-inline` na presença de nonce); hash `sha256` (o widget vem de endpoint dinâmico, então o hash quebraria sem aviso quando o provedor mudasse o conteúdo); `strict-dynamic` (afrouxa mais que o necessário — autorizaria o SDK a carregar qualquer script). Todas as invariantes de `spec/requests/security_headers_spec.rb` seguem valendo: nenhum curinga, `script-src` com nonce e sem `unsafe-inline`. Coberto por teste que exige que o nonce entregue ao Brick seja **o mesmo do cabeçalho** — verificado falhando com a view revertida. Pipeline verde: 536 Minitest, 429 RSpec, rubocop sem achados.

Segue o mesmo padrão do `style-src` de 2026-09-07 (o SDK injeta `<style>` sem nonce), com a diferença de que lá não havia opção e `unsafe-inline` foi inevitável. A lição estava registrada e não foi aplicada ao `script-src` porque **ninguém tinha chegado até aqui num browser**: o bloqueio só acontece quando o Brick tem um vendedor real para consultar, e as specs de CSP afirmam que o cabeçalho está bem formado, nunca que o SDK funciona sob ele.

BOTÃO DE CARTÃO NÃO RESPONDIA AO CLIQUE, corrigido em 2026-09-12 — **achado na primeira verificação num navegador real**. O botão "Pagar com cartão de crédito" ficava num `<div>` irmão do elemento que carrega `data-controller="payment-method-choice"`, e Stimulus só liga um `data-action` ao controller que **contém** o elemento: o clique não alcançava `showCard()`, o `<template>` nunca era clonado e a tela não reagia — sem erro no console, sem falha no servidor. A correção aninha a grade dos botões e a seção do Brick sob um único `data-controller` (a atribuição de `seller` subiu para antes do bloco, porque passou a ser lida nos dois ramos). **Por que 485 Minitest e 383 RSpec não pegaram**: as request specs afirmam que o botão existe no HTML, e ele existia; faltava alguém clicar. Coberto agora por `test/system/payment_method_choice_test.rb`, verificado falhando com a view revertida via `git stash` — guarda real, não decorativa. Pipeline verde: 535 Minitest, 429 RSpec, rubocop e brakeman sem achados.

**A CSP apertada em 2026-09-07 está CORRETA, agora observada e não só especificada.** Com o botão respondendo, o SDK carregou e desenhou o skeleton do Brick (as barras cinzas com a forma do formulário: número, validade/CVV, titular) — quem desenha esse placeholder é o próprio SDK, então ele executou sem nada bloqueado. Se faltasse domínio na lista de 2026-09-07, o bloqueio apareceria aqui e não apareceu. Conferido à parte que os domínios citados no bundle de `sdk.mercadopago.com/js/v2` (`api`, `api-static`, `secure-fields`, `http2.mlstatic`) estão todos cobertos, cada um na sua diretiva. **O Brick NÃO chega a renderizar os campos**: ele para no skeleton. **A causa registrada aqui — vendedor local fabricado (Public Key real, `mercado_pago_user_id` e tokens inventados no `runner`) — estava ERRADA, e foi corrigida no mesmo dia**: o travamento se repetiu em produção com vendedor real e tokens do OAuth. Era a CSP bloqueando o script inline do antifraude que o SDK injeta; ver o parágrafo "O BRICK ERA BLOQUEADO PELA CSP" acima. Segue válido que a Public Key de teste é aceita (`/v1/payment_methods`, HTTP 200 com 4 bandeiras de crédito). Descartados por medição, não por dedução: markup (botão, chave, valor, nonce e controller corretos no HTML entregue), Public Key (aceita pela API), CSP (domínios cobertos), alcance do SDK (283 KB, HTTP 200), valor (R$ 109,90 dentro da faixa R$ 0,50–60.000) e servidor (200 OK em 19 ms).

Fica **parcialmente fechada** a pendência "o Card Payment Brick nunca foi verificado visualmente": o que dependia do código da EloShop está verificado — o clique chega ao controller, o SDK carrega, a CSP sobrevive. O que falta depende de vendedor real e segue sendo a reconexão do `Ateliê do Mercado Pago` em produção, que grava chave e tokens coerentes entre si e permite ao Brick consultar um vendedor que existe. O teste de cartão contra o sandbox continua pendente pela mesma dependência.

RECONEXÃO DO MERCADO PAGO SEM DESPUBLICAR, em 2026-09-09. Vendedor conectado antes da Fase 24 não tem `mercado_pago_public_key` e precisa reautorizar, mas o banner do painel só mostrava "Conectar" quando NÃO havia conexão — o único caminho aparente para quem já estava conectado era "Desconectar" e conectar de novo, e desconectar força `pending` e suspende a publicação do catálogo. O modelo já distinguia os dois casos (`connect_mercado_pago!` só volta a `pending` quando o identificador da conta muda) e a rota de conectar já aceitava vendedor conectado; faltava expor isso na interface. Botão "Reconectar" adicionado em cinza, não vermelho, para não competir com o destrutivo — na paleta da loja o `brand` também é avermelhado. A invariante da aprovação preservada não tinha teste (só o caso oposto, da troca de conta) e agora tem, no Minitest; no RSpec, o botão presente quando conectado, ausente quando o OAuth não está configurado, e a autorização reiniciando para vendedor já conectado. Verificado que cada guarda falha ao reverter a mudança correspondente. O callback completo não foi exercitado localmente (exige o provedor real); a preservação da aprovação no retorno está coberta por teste de modelo, não por observação.

PRIMEIRA TENTATIVA DE PIX REAL NO SANDBOX, em 2026-09-08 — **falhou no gateway, mas validou o resto**. Pedido #21 criado em produção com o `Ateliê do Mercado Pago`: R$ 50 de produto + R$ 20 de frete = R$ 70, comissão de R$ 7,50 (15% sobre o produto, sem frete) e R$ 62,50 para o artesão — `SellerOrder`, split e aritmética corretos, exatamente o §34. O que falhou foi só a criação da cobrança: o Mercado Pago devolveu HTTP 500 com `user_allowed_only_in_test`. **A causa não é o pagador** (o código já resolve `payer_email` para a TESTUSER compradora, verificado em produção) **nem incompatibilidade entre contas de teste** (hipótese que levantei e descartei): consultando `GET /v1/payment_methods` com o token do artesão, a conta lista 19 métodos — cartão de crédito, débito, pré-pago, boleto e saldo — e **PIX não está entre eles**. **A conclusão tirada disso estava errada, e foi corrigida em 2026-09-09** (ver o parágrafo do dia): não é chave PIX faltando na conta, é que PIX não é pagável com credencial de teste — logo o bloqueio é MAIOR do que pareceu aqui, não menor. Segue válido que a conta **já aceita cartão**, e que o teste da Fase 24 depende só de reconectar o vendedor para gravar a `mercado_pago_public_key`. Dois defeitos reais corrigidos no caminho (ver o parágrafo seguinte). Erro de método registrado: deduzi a causa do nome do erro em vez de consultar `/v1/payment_methods`, que é leitura e responde direto — a segunda vez no mesmo dia em que uma dedução a partir de sinal indireto saiu errada.

FALHA DE COBRANÇA DEIXAVA O CLIENTE ESPERANDO PARA SEMPRE, corrigido em 2026-09-08. Quando a chamada ao gateway levanta, a tentativa **permanece `processing`** de propósito — a cobrança pode ter nascido do outro lado, e `Payments::Authorize` reusa a chave de idempotência para não cobrar duas vezes; `Payment#external_id` é obrigatório fora de `processing`, então não há outro estado onde "falhou antes de existir cobrança" caiba. Mas a tela lia todo `processing` como "vai chegar": mostrava "Preparando a cobrança. Aguarde alguns instantes." indefinidamente e seguia consultando um status que nunca mudaria. **Não é problema de sandbox** — em produção, qualquer instabilidade do gateway produziria o mesmo para um cliente real. `Payment#stalled?` (`PROCESSING_STALE_AFTER`, 2 min, generoso perto dos timeouts de 5s/15s) separa "ainda pode chegar" de "não vem mais": passado o limite a tela admite a falha, oferece "Tentar novamente" e para o polling, com o registro seguindo `processing` no banco e a proteção contra cobrança dupla intacta. **A primeira tentativa de correção foi revertida**: marcar `failed` no rescue parecia óbvio, mas quebrou o teste que garante o reuso da chave após timeout — o `processing` persistente é invariante, não descuido. Também tornados permanentes os dois logs marcados TEMPORÁRIO (foram eles que identificaram a causa) e `RequestFailed` passou a carregar o código de erro do gateway, mantendo o corpo da resposta fora da mensagem (§43): "respondeu 500" sozinho custou a investigação inteira. Pipeline local verde: 532 Minitest, 421 RSpec, rubocop sem achados.

ESTADO REAL DA INTEGRAÇÃO COM O MERCADO PAGO, apurado em 2026-09-08 — **o roadmap descrevia como pendente coisas já feitas**. Consultado o banco de produção via `railway ssh`: a aplicação Marketplace está configurada (`PAYMENT_GATEWAY=mercado_pago`, `MERCADO_PAGO_MARKETPLACE_APP_ID`, `CLIENT_SECRET`, `REDIRECT_URI` apontando para a rota real `/painel/mercado-pago/callback`, `SANDBOX=true`, `WEBHOOK_SECRET` e um `TEST_PAYER_EMAIL` de TESTUSER comprador), e **dois vendedores estão conectados**: `Ateliê da Ana` (`live_mode: true`, 2026-09-01) e `Ateliê do Mercado Pago` (`live_mode: false`, sandbox, 2026-09-06, login do painel `uriel@artesao.com.br`). A conexão de sandbox é a validação que o commit `0270d70` registrou em `docs/payments.md` — e que só se obtém executando o fluxo, já que ela é o que revelou as duas causas do `invalid_client` (PKCE ausente e Client Secret trocado pelo Access Token). **A pendência real é outra e é menor**: nenhum dos dois tem `mercado_pago_public_key`, porque ambos conectaram antes de a Fase 24 introduzir a coluna — reconectar (um clique no painel do artesão) grava a chave e libera o cartão; o PIX não depende dela. Erro de método registrado para não repetir: concluí "a conta TESTUSER não existe" porque `/admin/contas-de-teste-mercado-pago` estava vazio, mas essa tela é um **cofre opcional** de credenciais, não o registro do que existe no Mercado Pago — o estado da integração se apura no `Seller`, não nela. Nota de operação: `railway run` **não alcança** o banco de produção (roda local, e `postgres.railway.internal` só resolve dentro da rede da Railway — o erro enganoso é `Database not found: railway`); use `railway ssh --service eloshop-web`.

API v1 COM FORMATO JSON FIXO, em 2026-09-08. Fecha o débito registrado na expansão da API: `namespace :api, defaults: { format: :json }`. A medição corrigiu a descrição do próprio débito — clientes de API (curl sem `Accept`, `*/*`, `application/json`, `.json` na URL) **já respondiam 200**; o 406 era exclusivo de quem pedia HTML explicitamente, e o corpo devolvido era a página de exceção do Rails em HTML, não um erro JSON. Como a API v1 é pública, somente leitura e só tem views `.jbuilder`, não há segundo formato para negociar: a negociação de conteúdo não comprava nada e cobrava o preço justamente de quem estava conhecendo a API pelo navegador. O `swagger.yaml` foi regenerado e **não mudou** — a documentação já declarava `application/json`, o comportamento é que agora bate com ela. O teste do caso do navegador ficou fora das specs rswag de propósito (elas alimentam o OpenAPI, e "cliente pedindo HTML" não é documentação de contrato) e vive em `spec/requests/api/v1/content_negotiation_spec.rb`, que cobre também o 404 em JSON sob `Accept: text/html` e a não-regressão dos clientes que já funcionavam; verificado que 3 dos 4 exemplos falham com a rota revertida. Pipeline local verde: 529 Minitest, 418 RSpec, rubocop sem achados; os três endpoints conferidos no app real com o `Accept` do Chrome.

IDENTIDADE DA SESSÃO DE `User` NO MENU PÚBLICO, em 2026-09-08. Sintoma relatado: dois perfis do mesmo Chrome mostravam menus diferentes na loja, sem explicação aparente. A causa não era bug — era a regra em vigor: o admin não via nenhum link de ateliê **nem da administração**, que só era alcançável digitando `/admin` de cor. Duas correções. (1) O menu passa a dar **um destino por papel de `User`**: admin → "Administração", vendedor → "Painel do Artesão", os demais → login/cadastro de artesão. Esconder o link da administração não protegia nada — quem protege é o `require_admin!` (§38) — e só deixava o admin sem caminho pela interface; a spec que afirmava o contrário foi atualizada de propósito, não por quebra acidental. (2) O topo passa a anunciar a sessão de `User` (papel, quem está logado, e um "sair" próprio de cada porta: `session_path` para o admin, `seller_logout_path` para o vendedor), porque `User` e `Customer` são sessões independentes que podem estar ativas ao mesmo tempo no mesmo navegador — antes o header refletia só a segunda, e quem navegava como admin não tinha nenhum sinal disso. `ApplicationHelper#user_session_identity` é a fonte única dos rótulos/destinos, para o bloco do topo e o do hambúrguer não saírem de sincronia. O storefront de quem só compra não mudou. Pipeline local verde: 529 Minitest, 414 RSpec, rubocop e brakeman sem achados; os três papéis verificados no app real, autenticando de verdade nas duas portas de login. Achado colateral de ambiente, **não de código**: o banco de teste local estava com dados inconsistentes e produzia 15 falhas RSpec e 529 erros de fixture — reproduzidos com `git stash`, portanto pré-existentes; `db:drop db:create db:schema:load` devolveu as duas suítes ao verde.

API PÚBLICA v1 expandida em 2026-09-07. `Api::V1::BaseController` centraliza `allow_unauthenticated_access` e o 404 em JSON, que estavam duplicados no controller de produtos. A listagem de produtos ganhou os filtros `q`, `seller`, `category` e `availability` — um subconjunto dos filtros do catálogo do storefront, sobre o mesmo escopo `Product.publicly_visible`: **a API não expõe nada que a loja já não mostre**, e nenhum endpoint devolve dado de cliente ou de operação do vendedor. Filtrar por uma categoria pai traz também as subcategorias (via `Category::Tree`), e slug inexistente responde 200 com lista vazia, não 404 — é filtro, não recurso. A resposta passou a incluir `total_count`. Dois recursos novos, ambos somente leitura: `GET /api/v1/sellers` (só ateliês aprovados **com peça publicada**, mesmo critério da vitrine pública, com contagem em uma leitura agregada em vez de uma query por ateliê) e `GET /api/v1/sellers/{slug}`; e `GET /api/v1/categories`, a árvore visível com breadcrumb e slug do pai, excluindo categoria desabilitada e toda a subárvore abaixo dela. `preload` e não `includes` na listagem, pela regra de JIT da Fase 17. Achado durante a revisão, medido dentro de `ActiveRecord::Base.uncached` (regra da Fase 17): a listagem fazia **24 queries** numa página de 12 produtos, e o N+1 não vinha da view — vinha de `Product#available_for_purchase?`, que pergunta `category.visible?`, e isso sobe a árvore de categorias uma query por nível, por produto. `preload(category: :parent)` derrubou para 7, mesmo remédio já aplicado no catálogo do storefront. `/api/v1/sellers` responde em 4 queries e `/api/v1/categories` em 3, ambas independentes do tamanho do catálogo. Pipeline local verde: 519 Minitest, 405 RSpec (26 deles na API v1), rubocop e brakeman sem achados; `swagger/v1/swagger.yaml` regenerado e os quatro endpoints verificados no app real. Débito conhecido e **pré-existente**, não introduzido aqui: sem cabeçalho `Accept: application/json` a API responde 406, porque só existem views `.json` — vale decidir se `defaults: { format: :json }` na rota é o comportamento desejado. **Resolvido em 2026-09-08 (opção `defaults`), e a descrição acima estava imprecisa**: sem `Accept` nenhum a API já respondia 200; o 406 atingia só quem pedia HTML explicitamente — ou seja, quem colava a URL na barra do navegador — e ainda devolvia a página de exceção do Rails em HTML, não JSON. Ver o parágrafo do dia.

FRETE REAL (ADR 005), Etapa 1 — cotação — implementada em 2026-09-07. Cada artesão conecta a própria conta do Melhor Envio pelo painel via OAuth2 (sem PKCE, `client_id` numérico, e o token não devolve identificador de conta — diferenças confirmadas na doc oficial e registradas em `docs/shipping.md`), com tokens cifrados em `Seller` e salt separado do Mercado Pago. `Shipping::Calculator` deixou de ser tabela e virou orquestrador: cota no provedor quando há conta conectada e CEP de origem, cai na tabela interna (R$ 15 + R$ 5/kg) quando não há conexão, quando o provedor falha, estoura o timeout (3s/5s) ou não sobra opção — a tabela é fallback deliberado, não legado. Cache de 30 min por origem+destino+itens, porque o rate limit do provedor não é documentado. O checkout oferece a opção mais barata e a mais rápida; **o cliente escolhe por identificador e nunca por preço**, e `Checkout::CreateOrder` recota no servidor, recusando uma opção que não está mais entre as ofertadas. Pipeline local verde: 519 Minitest, 392 RSpec, rubocop e brakeman sem achados; fluxo de fallback validado ponta a ponta no app real, inclusive a recusa de uma opção forjada. A integração NUNCA rodou contra o sandbox do Melhor Envio (falta credencial) — os testes stubam HTTP e cobrem o contrato do adapter, não a API real. Etiqueta (Etapa 2) e webhook de rastreio (Etapa 3) seguem bloqueados pelas decisões de negócio do ADR 005.

O WAF DO MELHOR ENVIO BLOQUEIA O IP DA APLICAÇÃO, apurado em 2026-09-12 — a Etapa 1 **saiu do "falta credencial" e entrou no "bloqueado pelo provedor"**. Com a aplicação `11947` criada no sandbox e as quatro variáveis configuradas (`MELHOR_ENVIO_CLIENT_ID`/`CLIENT_SECRET`/`REDIRECT_URI`/`SANDBOX=true`), conectar pelo painel falha com "Melhor Envio recusou a vinculação (HTTP 403)". A causa é do provedor e tem código próprio: **`E-WAF-0003 — Acesso bloqueado`**, página HTML servida pelo `awselb/2.0`, ou seja, a requisição **para no load balancer e nunca chega à API**. A prova é a mesma requisição partindo de dois IPs: da estação de desenvolvimento (`179.82.57.136`) responde **401 `invalid_client` em JSON** (a API funcionando, recusando um secret propositalmente inválido); do contêiner da Railway (`152.55.184.216`) responde **403 HTML de WAF**. Mesmo payload, mesmo `client_id`, só o IP difere. Isso **descarta** credencial (daria 401 JSON — e a doc oficial do endpoint só documenta 200 e 401, 403 não é resposta prevista da API), escopo ou permissão da aplicação (viria JSON de erro OAuth), rate limit (5 chamadas seguidas com `x-ratelimit-remaining: 57` de 60), `User-Agent` (testado com e sem, 401 nos dois no IP liberado) e ambiente divergente. **Próximo passo é externo**: chamado pedindo a liberação do IP no WAF. Atenção ao risco de recaída — o IP de saída da Railway não é estático por contrato, então convém perguntar no chamado se há liberação por faixa/CIDR ou procedimento para integrações em nuvem; a alternativa durável é IP fixo de saída. **Toda a cadeia da EloShop está validada ponta a ponta** (botão, `state`/CSRF, sessão, callback, troca de token, tratamento de erro com caminho de volta); o que falta não é código.

DOIS DIAGNÓSTICOS ERRADOS ANTES DE CHEGAR NO WAF, ambos em 2026-09-12, **e os dois caíram por evidência direta**: (1) "conectou, o flash só passou rápido" — afirmado a partir de um `302` de sucesso no log, derrubado por screenshot do painel mostrando "Conecte sua conta"; o `302` era o `return` de um guard, não gravação. (2) "`SameSite=Lax` perdeu o cookie no retorno cross-site" — derrubada porque `Lax` permite GET top-level e porque o OAuth do Mercado Pago usa a mesma sessão, no mesmo tipo de retorno, e funciona. O que fechou a causa foi instrumentar duas vezes (PRs #82 e #83) e observar: o #82 provou que o `state` **passava** justamente por não registrar nada, e o #83 mostrou que o corpo do 403 não era JSON — pista de que respondia uma camada anterior à API. Quinta ocorrência do mesmo padrão nesta integração: contagem de queries, código de status e tempo de resposta são sinal indireto; screenshot, console e corpo da resposta são evidência. Vale registrar que `log_error_response` parou em `error: "corpo ilegível"` e o `E-WAF-0003` só apareceu via `railway ssh` — logar um trecho sanitizado do corpo quando não for JSON teria dado o código direto. **Melhoria FEITA**: no Melhor Envio pelo PR #84 e no gateway do Mercado Pago em 2026-09-13 (`9aa3157`), onde a mesma lacuna seguia aberta — o `rescue` do corpo não-JSON devolvia `nil` e nenhum evento era emitido. Os dois agora registram `content_type`, `server` e um trecho sanitizado, truncado em 300 caracteres e sem marcação (§43); a sanitização é varredura linear sem regex, porque `<[^>]*>` sobre corpo de terceiro é `rb/polynomial-redos` (CodeQL).
...
`Fase 24 (cartão de crédito): **tudo que depende do código da EloShop está feito e observado num navegador**; o que falta é externo. A verificação visual de 2026-09-12 rendeu cinco defeitos reais, todos corrigidos (#76 a #80) e nenhum detectável pelas suítes. O Brick renderiza, tokeniza o cartão no navegador e o backend chama o gateway; a falha é tratada com clareza para o cliente, com caminho de volta. **O bloqueio é o HTTP 500 do Mercado Pago em `/v1/payments`**, com `error: internal_error` e `cause: []` — resposta opaca, reproduzida com cartão de teste oficial e 1 parcela, e idêntica à que o PIX levou em 2026-09-09 na mesma conta. **Próximo passo: abrir chamado com o Mercado Pago**, com a evidência do parágrafo acima (conta `143360137`, endpoint, payload aceito, dois meios de pagamento, duas datas). Desde 2026-09-13 (`9aa3157`, no ar) o gateway também registra a camada que respondeu quando o corpo do erro não é JSON — se o 500 vier de uma camada anterior à API, como foi o caso do Melhor Envio, isso aparece agora no log em vez de sumir num `rescue` silencioso. Vale conferir o `payment.mercado_pago_gateway_http_error` de uma tentativa nova antes de abrir o chamado: pode mudar a quem endereçá-lo. **Terceira reprodução em 2026-09-13, 20:07:42 UTC (pedido #37, R$ 120,00, `Ateliê do Mercado Pago`)**: mesmo sintoma, agora com o log novo confirmando que o corpo **é** JSON válido — `error=null` (não string), `cause=[]` — então a melhoria do `9aa3157` não muda o diagnóstico aqui; ela cobre o caso de corpo não-JSON, que não é este. Confirma pela terceira vez, com conta e vendedor corretos, que a causa é do provedor. Para reproduzir, compre do `Ateliê do Mercado Pago` e escolha cartão — a Public Key está gravada desde 2026-09-12 e nenhuma reconexão é necessária. Enquanto isso não se resolver, o split e a comissão de 15% seguem **sem validação ponta a ponta** — são a única parte do §34 que nenhum caminho local exercita.

**Decisão de negócio pendente**: o antifraude (device profile) tenta carregar pixels de `www.mercadolibre.com` e `www.mercadolivre.com` que a CSP bloqueia (`img-src`), mais um `connect` a `/jms/lgz/background/etid`. O conteúdo é fingerprinting do dispositivo — a carga em base64 traz lista de plugins, presença de webcam e tipos MIME. Liberar autoriza essa coleta em domínio que hoje não está na política; não liberar deixa o antifraude degradado, com efeito desconhecido sobre recusa e chargeback; `advancedFraudPrevention: false` desliga a tentativa explicitamente, abrindo mão da proteção. Não impede o pagamento e não foi decidido.

No frete (ADR 005), a Etapa 1 está implementada e **a configuração já foi feita**: aplicação `11947` criada no sandbox, as quatro variáveis definidas na Railway e o fluxo OAuth exercitado em produção. A dependência que resta é **externa e diferente da anterior**: o WAF do Melhor Envio bloqueia o IP de saída da aplicação com `E-WAF-0003` (ver o parágrafo de 2026-09-12), então o próximo passo é o chamado pedindo liberação do IP `152.55.184.216` — e, no mesmo chamado, perguntar se há liberação por faixa/CIDR, porque o IP da Railway não é estático por contrato. Enquanto isso, a cotação real **nunca rodou**: o catálogo segue com a tabela interna, que é fallback deliberado e continua correta. As Etapas 2 (etiqueta) e 3 (webhook de rastreio) continuam travadas pelas decisões de negócio do ADR 005 — quem paga a etiqueta, se existe frete grátis, e quem absorve a diferença quando o frete real sai mais caro que o cobrado.
...
FINANCIALS DO ADMIN implementado em 2026-09-22. A barra administrativa ganhou a aba `/admin/financials`, restrita a administradores, com três blocos somente leitura: configuração local da integração Mercado Pago (gateway, OAuth, webhook, ambiente e último evento processado), movimentação financeira líquida calculada a partir dos snapshots de `SellerOrder` e situação das tentativas de pagamento Mercado Pago, e lista de artesãos que exigem ação. A tela deriva a disponibilidade de PIX/cartão e as pendências dos dados já persistidos, não chama o provedor ao abrir e nunca renderiza access token, refresh token, client secret ou segredo do webhook. Cobertura focada: acesso anônimo bloqueado, menu ativo, totais financeiros e ausência dos segredos no HTML. Pipeline local verde: 554 Minitest, 430 RSpec, RuboCop e Brakeman sem achados.

CONCILIAÇÃO FINANCEIRA DO MARKETPLACE implementada em 2026-09-22. A aba `/admin/financials` ganhou uma tabela que cruza pagamentos da EloShop com o Sales Report oficial do Mercado Pago e exibe venda, artesão, valor, comissão, tarifa do provedor, líquido do artesão e data de liberação. A atualização é manual e somente leitura: lista os `statements` existentes, baixa o CSV mais recente e guarda o resultado por 24 horas, sem criar estrutura, agenda ou relatório no provedor. A credencial nova é `MERCADO_PAGO_MARKETPLACE_ACCESS_TOKEN` (Access Token de produção da aplicação, distinto do Client Secret e dos tokens dos vendedores); quando ausente, a tela mostra os dados locais com a origem “Somente EloShop” e não tenta consultar a API. Como o Sales Report não contém a liberação, a atualização consulta `money_release_date` nos pagamentos individuais com o OAuth do artesão, limitada a 50 consultas por execução. Registros oficiais sem correspondência local também ficam visíveis. Testes focados cobrem parsing/caching do CSV, sigilo do token, os dois formatos documentados da listagem de statements, cálculo local, refresh manual e enriquecimento da liberação.
