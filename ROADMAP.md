...
`Fechar a Fase 24 (cartão de crédito). A verificação visual **destravou e já rendeu dois bugs reais**, ambos invisíveis para as suítes: o botão de cartão fora do escopo do controller Stimulus (clique que não chegava a lugar nenhum) e o script inline do antifraude bloqueado pela CSP (Brick preso no skeleton). A reconexão do `Ateliê do Mercado Pago` **foi feita em 2026-09-12 às 17:22**: `mercado_pago_public_key` gravada, identificador `143360137` preservado, ateliê segue aprovado e publicado. **Próximo passo: exercitar o cartão de teste ponta a ponta em produção**, com a correção do `deviceProfileCspNonce` no ar — pagar com `APRO` no titular e conferir que o pagamento é aprovado sincronamente, que o pedido é confirmado, que os três avisos de fan-out disparam e que a comissão de 15% cai certa no `SellerOrder` (o split é a parte que só este caminho exercita). Se o Brick ainda travar depois desta correção, o console é a primeira parada — não deduzir a causa de sinal indireto, erro já cometido quatro vezes nesta integração. Vale notar que o antifraude (device profile) estava sendo bloqueado em toda transação de cartão, não só em teste.
