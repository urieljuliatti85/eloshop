...
`Fechar a Fase 24 (cartão de crédito). Duas pendências saíram da lista: **apertar a CSP foi feito em 2026-09-07** (commit `eae9ba7`) e **observado funcionando num navegador em 2026-09-12** — o SDK carregou e desenhou o skeleton do Brick, sem nada bloqueado, o que a spec de invariantes não conseguia afirmar; e a **verificação visual está parcialmente feita**, tendo achado de imediato um bug real (o botão de cartão fora do escopo do controller Stimulus, corrigido no mesmo dia). Resta um único caminho, e ele é o mesmo para as duas frentes que sobraram: **reconectar o `Ateliê do Mercado Pago` em produção** pelo botão "Reconectar" do painel, que grava `mercado_pago_public_key` e tokens coerentes entre si. Só com um vendedor que existe do lado do provedor o Brick passa do skeleton e renderiza os campos — localmente ele trava ali porque a Public Key era real mas o `mercado_pago_user_id` e os tokens eram fictícios. Feita a reconexão, o fluxo de cartão contra o sandbox fica exercitável na sequência: a conta TESTUSER existente **já serve**, porque lista cartão entre os métodos disponíveis (o que faltou nela foi PIX).
