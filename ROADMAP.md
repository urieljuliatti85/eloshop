...
`Fase 24 (cartão de crédito): **tudo que depende do código da EloShop está feito e observado num navegador**; o que falta é externo. A verificação visual de 2026-09-12 rendeu cinco defeitos reais, todos corrigidos (#76 a #80) e nenhum detectável pelas suítes. O Brick renderiza, tokeniza o cartão no navegador e o backend chama o gateway; a falha é tratada com clareza para o cliente, com caminho de volta. **O bloqueio é o HTTP 500 do Mercado Pago em `/v1/payments`**, com `error: internal_error` e `cause: []` — resposta opaca, reproduzida com cartão de teste oficial e 1 parcela, e idêntica à que o PIX levou em 2026-09-09 na mesma conta. **Próximo passo: abrir chamado com o Mercado Pago**, com a evidência do parágrafo acima (conta `143360137`, endpoint, payload aceito, dois meios de pagamento, duas datas). Para reproduzir, compre do `Ateliê do Mercado Pago` e escolha cartão — a Public Key está gravada desde 2026-09-12 e nenhuma reconexão é necessária. Enquanto isso não se resolver, o split e a comissão de 15% seguem **sem validação ponta a ponta** — são a única parte do §34 que nenhum caminho local exercita.
