...
`FASE 17 — Performance CONCLUÍDA; FASE 19 — Observabilidade CONCLUÍDA; FASE 20 — Produção e deploy em andamento (Etapa A concluída, Etapa B parcial); FASE 22 — Fundação do marketplace em andamento; FASE 23 — Split de pedidos, pagamento e frete por vendedor concluída no código; FASE 24 — Pagamento via cartão de crédito em andamento (fluxo completo sem erro local e em produção desde 2026-09-22, aprovação final de pagamento ainda sem observação — ver parágrafo do dia).
...
PIX E CARTÃO SEM ERRO NO FLUXO COMPLETO, LOCAL E EM PRODUÇÃO, confirmado pelo usuário em 2026-09-22 — o HTTP 500 `internal_error` do Mercado Pago (parágrafos "CARTÃO EXERCITADO PONTA A PONTA EM PRODUÇÃO" e seguintes, 2026-09-12/13) não voltou a ocorrer: o checkout completa PIX e cartão sem erro tanto localmente quanto em `https://eloshop-web-production.up.railway.app/`. **Ressalva**: a confirmação cobre o fluxo (Brick carrega, tokeniza, o backend chama o gateway e responde sem erro) — ainda não há observação de uma aprovação final de pagamento ponta a ponta (pedido pago de fato, status confirmado, split e comissão de 15% observados na prática). Antes de marcar a Fase 24 como concluída, validar isso com uma cobrança real de valor baixo.
