...
* **Verificação visual do Brick FEITA em 2026-09-12; o bloqueio restante é do provedor** — o Brick renderiza os campos, tokeniza o cartão no navegador (`card_token` chega ao backend já filtrado, §43) e o backend chama `/v1/payments`. A cadeia da EloShop está validada ponta a ponta. O que falha é a criação da cobrança: **HTTP 500 com `error: internal_error` e `cause: []`**, resposta genérica que não indica campo inválido nem credencial. Reproduzido com o cartão de teste oficial e 1 parcela, e idêntico ao que o PIX levou em 2026-09-09 na mesma conta de teste (`143360137`) — dois meios de pagamento, duas datas, mesmo sintoma. Não deduzir a causa a partir disso: `cause: []` é a ausência da informação que a confirmaria. O caminho é chamado com o Mercado Pago. **O split e a comissão de 15% seguem sem validação ponta a ponta**, porque dependem de um pagamento que conclua.
