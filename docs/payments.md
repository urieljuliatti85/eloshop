...
* **PIX e cartão completam o fluxo sem erro, local e em produção, confirmado pelo usuário em 2026-09-22.** Nem o 500 `create_ti` original (2026-09-12/13) nem o 403 `Payer email forbidden` (2026-09-14/17) voltaram a ocorrer — o checkout conclui em `https://eloshop-web-production.up.railway.app/` sem o bloqueio que motivou o chamado WCS-50393. Não se sabe qual das configurações testadas (ou uma mudança do lado do provedor) resolveu, porque não há um novo diagnóstico isolando a causa — o chamado nunca foi fechado com uma explicação do suporte. **Ressalva importante**: a confirmação cobre o fluxo completar sem erro (Brick carrega, tokeniza, o backend chama o gateway e recebe resposta de sucesso) — ainda **não há observação de uma aprovação final de pagamento** (pedido chegando a `paid`/`authorized`, split e comissão de 15% conferidos na prática, como o pedido #21 chegou a fazer para os valores mas não para o gateway em 2026-09-08). Antes de considerar a Fase 24 concluída, validar com uma cobrança real de valor baixo e conferir `Seller` (`mercado_pago_test_account`, vendedor conectado) via `railway ssh`, porque esse estado muda por fora do repositório.

* **O 500 `create_ti` opaco voltou em 2026-09-24 (pedidos #48 e #49), sem nenhum deploy da EloShop entre eles.** Mesmo sintoma exato do incidente de 2026-09-12/13: `POST /v1/payments` responde `500`, `error: null`, `cause: []`, cartão de teste oficial (`5480 8328 0103 3311`, `APRO`). Reproduzido duas vezes seguidas no mesmo vendedor de sandbox (`Seller#5`, `mercado_pago_test_account: false`, `live_mode: false`) sem nenhuma mudança de configuração entre as tentativas — não é intermitência de uma única chamada, e não é o cartão de teste "ter mudado" (o cartão oficial é fixo, documentado pelo próprio Mercado Pago). O deploy ativo nos dois pedidos (`2d2a0b7`/`860e25c`, PRs #113/#114) não toca em `PaymentsController`, `Gateways::MercadoPago` nem checkout — descarta regressão de código como causa. Fica como estava em 2026-09-14: sem causa confirmada do lado do suporte, comportamento intermitente reaparecendo sem aviso. `Gateways::MercadoPago#log_error_response` ganhou um `body_excerpt` de fallback (PR #115, mesclado em 2026-09-24) para quando `error`/`message`/`cause` vierem vazios — ainda não observado se o corpo bruto do próximo 500 trará mais informação que os anteriores.

* **O ticket WCS-50393 foi encerrado automaticamente por inatividade em 2026-09-24, sem resposta do suporte sobre a volta do 500.** Mensagem padrão de encerramento ("Como não recebemos sua resposta até 24/09/2026, sua solicitação de suporte foi encerrada") — não é uma resolução, é fechamento por timeout do lado do Mercado Pago. O histórico de investigação de setembro (500 `create_ti`, depois 403 `Payer email forbidden`, as trocas de vendedor real/TESTUSER) continua sem causa raiz confirmada por eles. Qualquer reincidência a partir daqui precisa abrir um **chamado novo** — não é mais possível responder dentro do WCS-50393 — e deve referenciar esse ticket como histórico, para não repetir os passos de diagnóstico já feitos (application_fee descartado, formatação do payload descartada, configuração da EloShop auditada e descartada).

* **Quarta ocorrência do mesmo 500 opaco: pedido #50, 2026-09-24 12:59:06 UTC** — mesmo `Seller#5`, mesmo sintoma (`error: null`, `cause: []`), request completo em 635ms sem timeout. Desta vez com o `body_excerpt` de fallback do PR #115 ativo: veio `null` — **confirma que o corpo da resposta do Mercado Pago é genuinamente vazio além de `error`/`message`/`cause`**, não uma lacuna de captura do nosso lado. Não há mais nada a extrair do lado da EloShop; qualquer diagnóstico adicional depende do suporte do provedor olhando o request pelo `X-Request-Id`/timestamp. Ainda não foi aberto o chamado novo (o WCS-50393 não aceita mais resposta) — texto de abertura já redigido em conversa anterior, pendente de envio.

* **Resposta do suporte (chamado novo, 2026-09-24): fluxo confirmado como suportado; pediram `X-Request-Id`, corpo exato enviado (mascarado) e resposta completa de uma falha real para correlacionar do lado deles.** Confirmaram que Checkout Bricks + `POST /v1/payments` com OAuth do seller e `application_fee` é o fluxo documentado, e que o 500 não corresponde a nenhuma limitação conhecida — como já houve aprovações com a mesma conta, a leitura deles é inconsistência pontual, não restrição permanente. `Gateways::MercadoPago#log_error_response` ganhou captura de `request_id` (`X-Request-Id` do header de resposta) e `sent_body` (resumo mascarado do corpo enviado: presença do token do Brick, e-mail mascarado, `installments`, `transaction_amount`, `application_fee`, `external_reference`) — PR #123, mesclado em 2026-09-24. Achado no processo: `Rails.event.notify` filtra por nome de chave em qualquer profundidade do payload (`filter_parameters`, partial match em `token`/`email`), substituindo até um valor já mascarado por `[FILTERED]` — as chaves do hash evitam esses nomes de propósito.

* **Quinta ocorrência, já com a captura nova ativa: pedido #52, 2026-09-24 14:18:30 UTC.** Mesmo sintoma (`500`, `error: null`, `cause: []`, `body_excerpt: null`), 615ms sem timeout. Dados capturados e já enviados ao suporte: `X-Request-Id 72cdfde2-76f8-431c-9217-d6918850fa61`; corpo enviado `{transaction_amount: 1, application_fee: 0.15, installments: 1, external_reference: "52", token presente, payer.email mascarado}`; confirmado que o token vai no header `Authorization: Bearer` e que não houve troca de aplicação/seller entre a tokenização do Brick e a chamada a `/v1/payments`. Aguardando retorno do suporte com esses dados.
...
Os testes automatizados (Minitest/RSpec) stubam HTTP e verificam o contrato do adapter (o que envia, o que devolve, o que recusa), nunca a API real — isso não muda com o progresso abaixo. **A integração já rodou contra o sandbox real** (ver "Cartão de crédito (Fase 24)" acima): passou por um 500 opaco e um 403 `Payer email forbidden` investigados a fundo com o suporte do Mercado Pago (ticket WCS-50393), e desde 2026-09-22 o fluxo completa sem erro, local e em produção. O que falta é a observação de uma **aprovação final de pagamento** ponta a ponta — pedido chegando a `paid`/`authorized` por um pagamento de fato aceito pelo gateway, com split e comissão conferidos.
