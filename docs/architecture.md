...
**Alerta de métrica crítica sem plano Pro** (2026-09-24): monitores nativos de threshold da Railway (CPU/RAM/disco cruzando um limite → e-mail/Slack) exigem o plano Pro — o projeto está no plano atual, sem esse recurso na interface. `CheckRailwayMetricsJob` (recorrente a cada 10 minutos, `config/recurring.yml`) reaproveita o mesmo `RailwayMetricsReport` do painel e, via `Observability::RailwayAlertCheck`, publica no mesmo webhook do Slack já usado pelos alertas nativos (`SLACK_ALERT_WEBHOOK_URL`) quando:

* CPU > 80% de 1 vCPU (`CPU_USAGE` já vem como fração 0–1, não há teto de RAM fixo declarado no serviço para calcular percentual)
* Memória > 1 GB (limiar absoluto — uso normal observado fica em 0,14–0,22 GB)
* Taxa de erro HTTP > 5% na última hora

Limiares são constantes no service, não configuração de ambiente — ajustar exige um PR, não uma variável da Railway. Sem `SLACK_ALERT_WEBHOOK_URL` configurada, o job roda e não faz nada (mesmo padrão defensivo do relatório). Falha na chamada ao Slack nunca propaga exceção — só emite `observability.railway_alert_failed`.
