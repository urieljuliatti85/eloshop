...
A primeira medição agregada dos logs HTTP das 24 horas anteriores, excluindo `/up` e `/ready`, encontrou somente 20 requisições: 3 na home (p50 57 ms, p95 180 ms), nenhuma no catálogo e 20 no total (p50 13 ms, p95 180 ms; 11 respostas 2xx e 9 respostas 3xx). A amostra é insuficiente para concluir a Fase 17 e deve ser repetida quando houver tráfego orgânico relevante, sem gerar carga artificial em produção.
